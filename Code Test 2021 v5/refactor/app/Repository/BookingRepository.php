...
use Carbon\Carbon;
use DTApiapp\Events\JobWasCanceled;
...
//        $this->logger->addInfo('USER #' . Auth::user()->id . ' reopen booking #: ' . $jobid);
    ...
    public function distance($jobid, $distance, $time)
    {
    ...
    {
        Job::where('id', '=', $data['jobid'])->update(array('admin_comments' => $data['admincomment'], 'flagged' => $data['flagged'], 'session_time' => $data['session'], 'manually_handled' => $data['manually_handled'], 'by_admin' => $data['by_admin']));
    }
